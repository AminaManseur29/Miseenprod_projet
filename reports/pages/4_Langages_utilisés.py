...
###
# from src.data_analysis import plot_wordcloud
...
).generate_from_frequencies(languages_count)
...
top_languages20 = pd.DataFrame(
    top_languages20, columns=["Langage", "Count"], index=range(1, 21)
)

fig_lang = px.bar(
    top_languages20.sort_values(by="Count"),
    x="Count",
    y="Langage",
    orientation="h",
    text_auto=True,
...
df["LanguagesList"] = df["HaveWorkedWith"].apply(
    lambda x: [] if pd.isna(x) else x.split(";")
)

df["TopLanguagesCount"] = df["LanguagesList"].apply(
